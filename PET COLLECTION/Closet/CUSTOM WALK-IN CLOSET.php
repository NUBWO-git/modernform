...
// ดึงข้อมูลจากตาราง custom_walk_in_closet
$sql = "SELECT id, name, image, category, created_at FROM custom_walk_in_closet ORDER BY $orderBy";
$result = $conn->query($sql);

$images = [];
if ($result->num_rows > 0) {
   while ($row = $result->fetch_assoc()) {
      $imageURL = "./custom/uploads/" . $row['image'];
