...
// ดึงข้อมูลจากตาราง milly
$sql = "SELECT id, name, image, category, created_at FROM milly ORDER BY $orderBy";
$result = $conn->query($sql);

$images = [];
if ($result->num_rows > 0) {
   while ($row = $result->fetch_assoc()) {
      // เพิ่ม `/` ในเส้นทางของไฟล์ภาพ
      $imageURL = "Milly table/uploads/" . $row['image'];
