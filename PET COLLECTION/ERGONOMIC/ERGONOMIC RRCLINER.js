...
document.addEventListener("DOMContentLoaded", function () {
   ...
   function fetchDataAndUpdate(sortOrder) {
      fetch(`http://localhost/modernform/PET%20COLLECTION/ERGONOMIC/ERGONOMIC%20RRCLINER.php?sort=${sortOrder}`)
...
document.addEventListener('DOMContentLoaded', function () {
   ...
   function renderGallery(filteredProducts) {
      ...
      updateItemCount(categoryFiltered); // ส่ง categoryFiltered แทนที่ products

      // ตรวจสอบการแสดง Pagination
      if (categoryFiltered.length > itemsPerPage) {
         renderPagination(categoryFiltered.length);
      } else {
   ...
   function updateItemCount(filteredProducts) {
      const count = filteredProducts.length; // จำนวนสินค้าที่แสดงในหน้านี้
   ...
   function fetchProducts(category = null) {
      const url = new URL('http://localhost/modernform/PET%20COLLECTION/ERGONOMIC/ERGONOMIC%20RRCLINER.php');
      const params = {};
      ...
      fetch(url)
         ...
         .then(data => {
            ...
            renderGallery(products); // ใช้ renderGallery แทน displayProducts
         })
...
document.addEventListener("DOMContentLoaded", function () {
   ...
   async function fetchImages() {
      try {
         ...
         const response = await fetch("http://localhost/modernform/PET%20COLLECTION/ERGONOMIC/ERGONOMIC%20RRCLINER.php");
   ...
   function renderPagination() {
      ...
      // ถ้ามีหน้ามากกว่าหนึ่งหน้า จึงจะแสดงปุ่ม pagination
      if (totalPages > 1) {
         // ปุ่ม Previous
         const prevButton = document.createElement("span");
         prevButton.innerHTML = '<span class="material-icons">chevron_left</span>'; // ไอคอนลูกศรซ้าย
   ...
   elements.forEach(function (element) {
      element.addEventListener("click", function () {
         ...
         if (element.tagName === "P") {
            ...
            targetUrl = "http://localhost/modernform/PET%20COLLECTION/Recliner/Recliner.html";
         }
