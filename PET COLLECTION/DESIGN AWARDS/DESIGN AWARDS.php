...
// ดึงข้อมูลจากตาราง design_awards
$sql = "SELECT id, name, image, category, created_at FROM design_awards ORDER BY $orderBy";
$result = $conn->query($sql);

$images = [];
if ($result->num_rows > 0) {
   while ($row = $result->fetch_assoc()) {
      $imageURL = "DESIGN AWARDS table/uploads/" . $row['image'];
