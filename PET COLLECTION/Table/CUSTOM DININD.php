...
// ดึงข้อมูลจากตาราง custom_dinind_table
$sql = "SELECT id, name, image, category, created_at FROM custom_dinind_table ORDER BY $orderBy";
$result = $conn->query($sql);

$images = [];
if ($result->num_rows > 0) {
   while ($row = $result->fetch_assoc()) {
      $imageURL = "./CUSTOM DININD TABLE/uploads/" . $row['image'];
