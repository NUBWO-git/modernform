...
document.addEventListener("DOMContentLoaded", function () {
   ...
   function fetchDataAndUpdate(sortOrder) {
      fetch(`http://localhost/modernform/Refeerece/Project%20Reference.php?sort=${sortOrder}`)
         ...
         .then(data => {
            ...
            gallery.innerHTML = '';
            renderGallery(currentPage); // แสดงผล
            renderPagination();
         })
...
document.addEventListener("DOMContentLoaded", function () {
   ...
   const defaultTitle = "Project Reference"; // ชื่อเมนูหน้าหลัก
...
document.addEventListener('DOMContentLoaded', function () {
   ...
   function renderGallery(filteredProducts) {
      ...
      updateItemCount();  // อัพเดทจำนวนสินค้าแสดง
   ...
   function fetchProducts(category = null) {
      const url = new URL('http://localhost/modernform/Refeerece/Project%20Reference.php');
      const params = {};
...
document.addEventListener("DOMContentLoaded", function () {
   ...
   async function fetchImages() {
      try {
         ...
         const response = await fetch("http://localhost/modernform/Refeerece/Project%20Reference.php");
   ...
   function renderGallery() {
      ...
      for (let i = start; i < end && i < totalItems; i++) {
         ...
         nameContainer.append(nameElement); // จัดเรียงชื่อสินค้าโดยไม่ใส่ปุ่ม
      ...
      // อัปเดตจำนวนสินค้าที่แสดงบนหน้า
      updateItemCount();
   }
   ...
   elements.forEach(function (element) {
      element.addEventListener("click", function () {
         ...
         if (element.tagName === "P") {
            ...
            targetUrl = "http://localhost/modernform/PET%20COLLECTION/CHATEAU/CHATEAU.html";
         } 
